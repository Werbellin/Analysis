...
class EventSelection :
    ...
    def GetEfficencies(self) :
        ...
        log.info("The following datasets are considered as signal:")
        for item in SignalData :
            log.info(item.name)
        log.info("The following datasets are considered as backgrounds: ")
        ...
        template = "{0:55}|{1:25}"
        log.info(str_single)
        log.info(template.format("Cut name", "S/√B @ " + "{:3.2f}".format(self.luminosity) + "fb⁻¹"))
        log.info(str_single)
        ...
        for tuple in significanceFormated :
            log.info(template.format(*tuple))
        log.info(str_double)
    ...
    def PrintCutflow(self) :
        ...
        log.info(str_double)
        for data in self.dataset :
            log.info("Cutflow for the %s dataset:", data.name)
            template = "{0:60}|{1:15}|{2:15}|{3:15}|{4:15}" # column widths: 8, 10, 15, 7, 10
            log.info("%s",str_single)
            log.info(template.format("Cut name", "Passed","@"+ "{:3.1f}".format(self.luminosity) + "fb-1", "abs.diff", "efficency")) # header
            log.info(str_single)
    ...
    def Finalize(self, plot_normalization = "LUMI") :
        ...
        #self.currentDataset = dataset_name
            ...
            for hist in self.Histos[data.name] :
                ...
                if hist._normalization == "CORR" :
                    title = hist.GetTitle()
                    title += str(hist.GetCorrelationFactor())
                    hist.SetTitle(title)
                    hist.Draw()
